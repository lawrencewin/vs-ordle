...
export function bindGameToSocket(io: Server, socket: Socket) {
    ...
    const endGameIfNeeded = (lobbyId: string) => {
        ...
        if (remainingPlayers.length === 1) {
            ...
            const remaining = game.players[pid]
            // it would suck if you lost the game because someone rage quit :(
    ...
    const removeFromLobby = (lobbyId: string, pid: PlayerID) => {
        ...
        } else if (GAMES[lobbyId].status === "playing") {
        ...
        (
            ...
            if (playerIsAlreadyInGame(pid)) {
                ...
                callback(null, {
                    ...
                    message:
                        "Given lobby ID doesn't currently exist on the server.",
                })
            } else {
    ...
    socket.on(
        "guess",
        async (guess: string, callback: (result: GuessResult) => void) => {
